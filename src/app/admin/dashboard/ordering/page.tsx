...
export default function OrderingSettingsPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (cachedData?.restaurantData) {
          ...
          console.log('[Ordering Page] Using cached restaurant data:', rest.name);
          setRestaurant(rest);
          setFormData({
          ...
          console.log('[Ordering Page] API response:', { status: data.status, hasData: !!data.data });
          ...
          if (data.status === 'success' && data.data) {
            ...
            setFormData({
              ...
              delivery_charge_max: rest.delivery_charge_max ?? 0,
              delivery_instruction: rest.delivery_instruction ?? '',
              enabled_services: rest.enabled_services ?? '["dinein", "takeaway", "delivery"]',
            });
            setIsLoading(false);
          } else {
  ...
  const handleServiceToggle = (service: string) => {
    try {
      let currentServices: string[] = JSON.parse(formData.enabled_services || '[]');
  ...
  if (isLoading) return <div className="p-6"><ProfileSkeleton /></div>;
  ...
  return (
    <div>
      <FullscreenLoader 
        isVisible={isSaving} 
        messages={["Saving settings...", "Updating ordering configuration...", "Almost done..."]} 
      />
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center gap-3">
               ...
               <div>
                  <h1 className="text-2xl font-bold text-gray-900">Ordering & Billing</h1>
                  <p className="text-gray-600 mt-1">Configure GST, delivery charges, and services</p>
               </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="flex flex-wrap gap-4">
                {['dinein', 'takeaway', 'delivery'].map((service) => (
                  ...
                  >
                    {service === 'dinein' && <Icons.BiSolidDish className="w-5 h-5" />}
                    {service === 'takeaway' && <Icons.Store className="w-5 h-5" />}
                    {service === 'delivery' && <Icons.MapPin className="w-5 h-5" />}
                    {service}
                  </button>
                ))}
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-6">
                ...
                {formData.delivery_charges_type === 'fixed' ? (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Fixed Delivery Charge (₹)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Delivery Instruction Text</label>
                  <textarea
