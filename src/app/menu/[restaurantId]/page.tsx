          ...
          categories.map(category => (
            ...
            >
              ...
              >
                ...
                <motion.div animate={{ rotate: openCategories[category] ? 180 : 0 }}>
                ...
                {openCategories[category] && (
                  ...
                  >
                    {groupedAndFilteredDishes[category].map((dish, index) => (
                      <div
                        key={dish.id}
                        className={`flex p-4 ${index < groupedAndFilteredDishes[category].length - 1 ? 'border-b border-gray-100' : ''}`}
                      >
                        <div className="flex-1 min-w-0 pr-4">
                          <h4 className="text-base font-bold text-gray-800 flex items-center gap-2">
                            <div className={`w-4 h-4 border ${dish.isVeg ? 'border-green-600' : 'border-red-600'} flex items-center justify-center`}>
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-2">
                            {dish.variations.map((v, i) => (
                              <div key={i} className="bg-gray-100 px-2 py-0.5 rounded">
                                <span className="text-xs font-semibold text-gray-700 capitalize">{v.size}</span>
                                <span className="text-xs font-bold text-orange-600 ml-1">₹{v.price}</span>
                              </div>
                            ))}
                          </div>
        ...
        <AnimatePresence mode="wait">
          {isCategoryModalOpen ? (
            ...
            <motion.div
              ...
              className="flex items-center gap-1"
            >
              <FiMenu className="w-5 h-5" /> {/* Placeholder for Fork/Spoon */}
              <span>Menu</span>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center sm:justify-center">
          <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl">
            <div className="flex items-center justify-between p-4 border-b">
              ...
              <button onClick={() => setShowFilterModal(false)}><FiX /></button>
            </div>
            <div className="p-4">
              <button
                onClick={() => setShowVegOnly(!showVegOnly)}
                className={`w-full p-4 rounded-lg border-2 ${showVegOnly ? 'border-green-500' : ''}`}
              >
                Veg Only
                {showVegOnly && <FiCheck />}
              </button>
            ...
            <div className="p-4 border-t">
              <button onClick={() => setShowFilterModal(false)} className="w-full bg-orange-500 text-white py-3 rounded-lg">Apply</button>
            </div>
          </div>
        </div>
      )}
